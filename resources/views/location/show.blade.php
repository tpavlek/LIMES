...
@section('content')
    ...
    @forelse($location->posts as $post)
        <div class="relative">
            ...
            @else
                <div class="border-green-dark shadow border-2 absolute pin-t pin-l ml-8 -mt-6 h-16 w-16 rounded-full text-center inline z-40 bg-center bg-cover" style='background-image: url("{{ $post->author->getImage() }}");'>
                </div>
            @endif

            <div class="border-b border-l border-r shadow mx-4 bg-white mb-8 relative z-10">
                <div class="pt-2 text-sm font-bold  tracking-wide text-grey-dark" style="margin-left:6rem;">{{ strtoupper($post->author->name) }}</div>
                <p class="p-4 pt-8 text-left leading-loose">
                    {{ $post->body }}
                </p>
